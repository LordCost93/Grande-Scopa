...
public class Client {
	...
	/*		if(validInput(read)){
			...
			if(true){
				String command=CodificaClient.codifica(read,numRoom,id,statoAttuale);
				//invio l'ordine al server
				ni.getCommand().handleCommand(command);//DOPO CHE FILTER MANDA MESSAGGI, LA COMPUTAZIONE RIPARTE DA QUA CON LA READLINE!!
			}
	 ...
	 */
	public void setStatoAttuale(StatiGioco statoAttuale) throws RemoteException {// direi di mettere i messaggi all'utente tutti in filter con notify message, qua non ci deve essere la view.
